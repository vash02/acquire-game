...
public class LargestAlphaStrategy extends PlayerStrategy {
    ...
    public List<Object> playTurn(State state) {
        ...
        playTurnresult = new ArrayList<>(3);
        while (!executed) {
            executed = true;
    ...
    public List<String> buyStrategyStocks(State state) {
        ...
        for(int i=1;i<hotelsCount.length;i++){
    ...
    public Tile nextTile(State state) {
        ...
        Collections.sort(playerTiles, Comparator.comparing(Tile::getRow).thenComparing(Tile::getColumn).reversed());
    ...
    public int getHotelNodeFilter(State state){
        ...
        switch (operation) {
            case "found":
                ...
                Hotel lastHotel = hotelFilterList.get(hotelFilterList.size() - 1);
                hotelFilter = board.getHotelIndex(lastHotel.getLabel());
                break;
            case "grow":
